...
productApp.get('/getproducts/:category',expressAsyncHandler(async(request,response)=>{
    
    //get productCollectionObject
    let productCollectionObject = request.app.get("productCollectionObject");
    
    //get productId from url parameter
    let pcategory = request.params.category;
    
    //get product by id
    let product = await productCollectionObject.find({category:pcategory}).toArray();
...
productApp.post('/addproduct',expressAsyncHandler(async(request,response)=>{
...
//create a route to handle '/updateproduct'
